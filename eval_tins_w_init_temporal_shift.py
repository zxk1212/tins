...
TEMPORAL_SHIFT_DATASET_ALIASES = {
    "I": "iNaturalist",
...
TEMPORAL_SHIFT_DATASET_SHORT_NAMES = {
    "iNaturalist": "I",
    "SUN": "S",
    "places365": "P",
    "dtd": "T",
}
TEMPORAL_SHIFT_DATASET_DISPLAY_NAMES = {
    "iNaturalist": "iNaturalist",
    "SUN": "SUN",
    "places365": "Places",
    "dtd": "Textures",
...
def process_args():
    ...
    parser.add_argument(
        "--eval-protocol",
        default="temporal_shift",
        type=str,
        choices=["temporal_shift", "four_ood", "openood_imagenet1k"],
        help="Evaluation protocol. Temporal shift keeps the dynamic extra-negative bank across consecutive OOD stages.",
    )
    parser.add_argument(
        "--openood-root",
    ...
    parser.add_argument(
        ...
        default="I->S->P->T",
        type=str,
        help="Temporal OOD order. Supports aliases like I->S->P->T or full names like iNaturalist,SUN,Places,Textures.",
    )
    parser.add_argument(
        "--bank-buffer-size",
        default=2000,
        type=int,
        help=(
            "Capacity of the temporal-shift overflow buffer. Features dropped from the active bank are "
            "kept here; once the buffer reaches this size, it is shuffled together with the bank and a "
    ...
    parser.add_argument(
        ...
        dest="use_buffer",
        action="store_true",
        help="Enable the temporal-shift overflow buffer.",
    )
    parser.add_argument(
        "--no-use-buffer",
        dest="use_buffer",
        action="store_false",
        help="Disable the temporal-shift overflow buffer.",
    )
    ...
    args.temporal_order_datasets = parse_temporal_order(args.temporal_order)
    args.temporal_order_tag = format_temporal_order(args.temporal_order_datasets, separator="->")
    args.temporal_order_key = format_temporal_order(args.temporal_order_datasets, separator="_")
...
def parse_temporal_order(order_spec):
    tokens = [token for token in re.split(r"[^A-Za-z0-9]+", order_spec.strip()) if token]
    if not tokens:
        raise ValueError("Temporal order is empty.")

    resolved = []
    for token in tokens:
        dataset_name = TEMPORAL_SHIFT_DATASET_ALIASES.get(token.upper())
        if dataset_name is None:
            raise ValueError(
                f"Unsupported temporal-order token '{token}'. "
                "Use I/S/P/T or dataset names such as iNaturalist, SUN, Places, Textures."
            )
        resolved.append(dataset_name)

    if len(resolved) != len(FOUR_OOD_DATASETS):
        raise ValueError(
            f"Temporal order must contain exactly {len(FOUR_OOD_DATASETS)} stages, got {len(resolved)}: {order_spec}"
        )
    if len(set(resolved)) != len(FOUR_OOD_DATASETS):
        raise ValueError(f"Temporal order must contain each OOD dataset exactly once: {order_spec}")
    return resolved


def format_temporal_order(dataset_names, separator="->"):
    return separator.join(TEMPORAL_SHIFT_DATASET_SHORT_NAMES[name] for name in dataset_names)


def build_temporal_shift_eval_setup(args, preprocess):
    id_loader = set_val_loader(args, preprocess)
    eval_specs = []
    for stage_idx, dataset_name in enumerate(args.temporal_order_datasets, start=1):
        short_name = TEMPORAL_SHIFT_DATASET_SHORT_NAMES[dataset_name]
        eval_specs.append(
            {
                "dataset_name": dataset_name,
                "display_name": TEMPORAL_SHIFT_DATASET_DISPLAY_NAMES[dataset_name],
                "group_name": "temporal_shift",
                "stage_index": stage_idx,
                "stream_name": f"temporal_shift_{short_name}",
...
def build_eval_setup(args, preprocess):
    if args.eval_protocol == "temporal_shift":
        return build_temporal_shift_eval_setup(args, preprocess)
    if args.eval_protocol == "four_ood":
        ...
        for dataset_name in FOUR_OOD_DATASETS:
            eval_specs.append(
                {
                    "dataset_name": dataset_name,
                    "display_name": TEMPORAL_SHIFT_DATASET_DISPLAY_NAMES.get(dataset_name, dataset_name),
                    "group_name": "all",
...
def maybe_expand_dynamic_bank(
    ...
    with torch.no_grad():
        ...
        #    超出的样本作为“溢出”进入对抗 temporal shift 的缓冲区。
        ...
        if args.use_buffer and buffer_capacity > 0 and buffer_features.shape[0] >= buffer_capacity:
...
def compute_tins_scores_from_image_features(
    ...
    class_prototypes,
    base_sim,
    initial_bank_features=None,
    initial_bank_scores=None,
    initial_buffer_features=None,
    initial_buffer_scores=None,
    return_bank_state=False,
):
    ...
    if initial_bank_features is None:
        bank_features = torch.zeros((0, feature_dim), dtype=torch.float16)
    else:
        bank_features = initial_bank_features.detach().cpu().to(torch.float16)
    if initial_bank_scores is None:
        bank_scores = torch.zeros(0, dtype=torch.float32)
    else:
        bank_scores = initial_bank_scores.detach().cpu().to(torch.float32)
    if initial_buffer_features is None:
        buffer_features = torch.zeros((0, feature_dim), dtype=torch.float16)
    else:
        buffer_features = initial_buffer_features.detach().cpu().to(torch.float16)
    if initial_buffer_scores is None:
        buffer_scores = torch.zeros(0, dtype=torch.float32)
    else:
        buffer_scores = initial_buffer_scores.detach().cpu().to(torch.float32)
    all_scores = []
    ...
    if return_bank_state:
        return scores, bank_features, bank_scores, buffer_features, buffer_scores
    return scores
...
def eval_mixed_stream_one_ood(
    ...
    score_outputs = compute_tins_scores_from_image_features(
        image_features=image_features,
        args=args,
        model=model,
        positive_features=positive_features,
        ...
        class_prototypes=class_prototypes,
        base_sim=base_sim,
        initial_bank_features=initial_bank_features,
        initial_bank_scores=initial_bank_scores,
        initial_buffer_features=initial_buffer_features,
        initial_buffer_scores=initial_buffer_scores,
        return_bank_state=return_bank_state,
    )
    if return_bank_state:
        (
            scores,
...
def main():
    ...
    dataset_rows = []
    grouped_results = {}
    dynamic_bank_features = None
    dynamic_bank_scores = None
    dynamic_buffer_features = None
    dynamic_buffer_scores = None

    if args.eval_protocol == "temporal_shift":
        log.debug(f"Temporal shift order: {args.temporal_order_tag}")
        log.debug(f"Bank buffer size for temporal shift: {args.bank_buffer_size}")

    for spec in eval_specs:
        out_dataset = spec["dataset_name"]
        display_name = spec.get("display_name", out_dataset)
        group_name = spec["group_name"]
        stream_name = spec["stream_name"]
        ood_loader = spec["ood_loader"]
        stage_index = spec.get("stage_index")
        if stage_index is not None:
            current_bank_size = 0 if dynamic_bank_features is None else int(dynamic_bank_features.shape[0])
            current_buffer_size = 0 if dynamic_buffer_features is None else int(dynamic_buffer_features.shape[0])
            log.debug(
                "Evaluating temporal stage %d/%d: %s (stream=%s, initial_dynamic_bank=%d, initial_buffer=%d)",
                stage_index,
                len(eval_specs),
                display_name,
                stream_name,
            ...
            (
                mixed_in_score,
                out_score,
                dynamic_bank_features,
                dynamic_bank_scores,
                dynamic_buffer_features,
                dynamic_buffer_scores,
            ) = eval_mixed_stream_one_ood(
                args=args,
                model=net,
                ...
                log=log,
                initial_bank_features=dynamic_bank_features,
                initial_bank_scores=dynamic_bank_scores,
                initial_buffer_features=dynamic_buffer_features,
                initial_buffer_scores=dynamic_buffer_scores,
                return_bank_state=True,
            )
            log.debug(
                "Finished temporal stage %d/%d: %s (final_dynamic_bank=%d, final_buffer=%d)",
                stage_index,
                len(eval_specs),
                display_name,
                int(dynamic_bank_features.shape[0]),
                int(dynamic_buffer_features.shape[0]),
            )
        else:
            log.debug(f"Evaluating OOD dataset {display_name} (group={group_name}, stream={stream_name})")
        ...
        method_name = args.score if stage_index is None else f"{args.score}_stage{stage_index}"
        print_measures(log, auroc, aupr, fpr, method_name=method_name)
        result_row = {
            "name": display_name,
            "group": group_name,
            "fpr": fpr,
