...
OPENOOD_CIFAR_GROUPS = {
    "CIFAR10": {
        "nearood": ["cifar100", "tin"],
        "farood": ["mnist", "svhn", "textures", "places365"],
    },
    "CIFAR100": {
        "nearood": ["cifar10", "tin"],
        "farood": ["mnist", "svhn", "textures", "places365"],
...
OPENOOD_DISPLAY_NAMES = {
    "cifar10": "CIFAR10",
    "cifar100": "CIFAR100",
    "tin": "TIN",
    "mnist": "MNIST",
    "svhn": "SVHN",
    "textures": "Texture / DTD",
    "places365": "Places365",
    "ssb_hard": "SSB Hard",
    "ninco": "NINCO",
    "inaturalist": "iNaturalist",
    "openimageo": "OpenImage-O",
}
...
def process_args():
    parser = argparse.ArgumentParser(
        description="Evaluate tins for ImageNet/CIFAR mixed-stream OOD detection on Four-OOD or OpenOOD protocols.",
    ...
    parser.add_argument(
        ...
        default="four_ood",
        type=str,
        choices=["four_ood", "openood_imagenet1k", "openood_cifar", "openood_cifar10", "openood_cifar100"],
        help="Evaluation protocol. OpenOOD modes evaluate each OOD dataset via its own mixed ID+OOD stream.",
    )
    parser.add_argument(
        "--openood-root",
        default=DEFAULT_OPENOOD_ROOT,
        type=str,
        help="Root directory of the OpenOOD repository used by the OpenOOD protocols.",
    )
    parser.add_argument(
        "--wordnet-dir",
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Optional official train imglist used to build ID class prototypes. If missing, fall back to the first N images per class from ImageFolder.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="Prompt template for ID labels.",
    )
    parser.add_argument(
        "--noun-prompt",
        default=OFFICIAL_NOUN_PROMPT,
    ...
    if args.eval_protocol == "openood_cifar10":
        args.eval_protocol = "openood_cifar"
        args.in_dataset = "CIFAR10"
    elif args.eval_protocol == "openood_cifar100":
        args.eval_protocol = "openood_cifar"
    ...
    if args.eval_protocol in {"four_ood", "openood_imagenet1k"} and args.in_dataset != "ImageNet":
        raise ValueError(f"{args.eval_protocol} only supports --in_dataset ImageNet.")
    if args.eval_protocol == "openood_cifar" and args.in_dataset not in OPENOOD_CIFAR_GROUPS:
        raise ValueError("--eval-protocol openood_cifar only supports --in_dataset CIFAR10 or CIFAR100.")
    ...
    if args.eval_protocol in {"openood_imagenet1k", "openood_cifar"}:
...
def resolve_train_imglist(args):
    ...
    repo_root = Path(__file__).resolve().parent
    if args.eval_protocol == "openood_cifar":
        openood_root = resolve_openood_root(args)
        benchmark_dir = resolve_openood_cifar_benchmark_dir(openood_root, args.in_dataset)
        if args.train_imglist is None:
            return materialize_openood_cifar_train_imglist(
                benchmark_dir=benchmark_dir,
                in_dataset=args.in_dataset,
                shot_per_class=args.train_shot_per_class,
                output_root=Path(args.cache_dir) / "openood_cifar_train_imglists",
            )
        candidates.append(benchmark_dir / OPENOOD_CIFAR_IMGLISTS[args.in_dataset]["train"])
    else:
        candidates.extend(
            [
...
def materialize_openood_cifar_train_imglist(benchmark_dir, in_dataset, shot_per_class, output_root):
    source_imglist = benchmark_dir / OPENOOD_CIFAR_IMGLISTS[in_dataset]["train"]
    if not source_imglist.is_file():
        raise FileNotFoundError(f"OpenOOD train imglist does not exist: {source_imglist}")

    output_root = Path(output_root)
    output_root.mkdir(parents=True, exist_ok=True)
    source_sig = compute_file_signature(source_imglist)[:12]
    output_path = output_root / f"{in_dataset.lower()}_{shot_per_class}shot_{source_sig}.txt"
    if output_path.is_file():
        validate_openood_cifar_train_imglist(output_path, in_dataset, shot_per_class)
        return output_path

    selected_lines = []
    class_counts = {}
    with source_imglist.open("r") as handle:
        for raw_line in handle:
            ...
            parts = line.split()
            if len(parts) < 2:
                raise ValueError(f"Invalid imglist line in {source_imglist}: {raw_line!r}")
            label = int(parts[1])
            current = class_counts.get(label, 0)
            if current >= shot_per_class:
                continue
            selected_lines.append(line)
            class_counts[label] = current + 1

    expected_classes = len(CIFAR10_CLASS_NAMES) if in_dataset == "CIFAR10" else len(CIFAR100_CLASS_NAMES)
    validate_classwise_fewshot_counts(
        class_counts=class_counts,
        expected_classes=expected_classes,
        shot_per_class=shot_per_class,
        imglist_path=source_imglist,
    )
...
def validate_classwise_fewshot_counts(class_counts, expected_classes, shot_per_class, imglist_path):
    if len(class_counts) != expected_classes:
        raise ValueError(
            f"Expected {expected_classes} classes in {imglist_path}, but found {len(class_counts)} after few-shot selection."
        )
    invalid_labels = sorted(label for label, count in class_counts.items() if count != shot_per_class)
    if invalid_labels:
        raise ValueError(
            f"Imglist {imglist_path} does not provide exactly {shot_per_class} samples for labels {invalid_labels[:10]}."
        )


def validate_openood_cifar_train_imglist(imglist_path, in_dataset, shot_per_class):
    expected_classes = len(CIFAR10_CLASS_NAMES) if in_dataset == "CIFAR10" else len(CIFAR100_CLASS_NAMES)
    class_counts = {}
    with Path(imglist_path).open("r") as handle:
        for raw_line in handle:
            line = raw_line.strip()
            if not line:
                continue
            parts = line.split()
            if len(parts) < 2:
                raise ValueError(f"Invalid imglist line in {imglist_path}: {raw_line!r}")
            label = int(parts[1])
            class_counts[label] = class_counts.get(label, 0) + 1
    validate_classwise_fewshot_counts(class_counts, expected_classes, shot_per_class, imglist_path)
    expected_total = expected_classes * shot_per_class
    total_count = sum(class_counts.values())
    if total_count != expected_total:
        raise ValueError(f"Expected {expected_total} samples in {imglist_path}, but found {total_count}.")
...
def resolve_openood_cifar_benchmark_dir(openood_root, in_dataset):
    benchmark_root = Path(openood_root) / "data" / "benchmark_imglist"
    for directory_name in (in_dataset.lower(), f"{in_dataset.lower()}"):
        candidate = benchmark_root / directory_name
        if candidate.is_dir():
            return candidate
    raise FileNotFoundError(
        f"Could not find OpenOOD CIFAR benchmark imglist directory for {in_dataset} under {benchmark_root}"
    )


def get_id_class_names(args, loader=None):
    if args.in_dataset == "CIFAR10":
        return CIFAR10_CLASS_NAMES
    if args.in_dataset == "CIFAR100":
        return CIFAR100_CLASS_NAMES
    return list(get_test_labels(args, loader))


def build_subset_imagefolder_loader(dataset_root, preprocess, batch_size, max_count):
    dataset = tv_datasets.ImageFolder(str(dataset_root), transform=preprocess)
    classwise_count = {}
    indices = []
    for index, label in enumerate(dataset.targets):
        current = classwise_count.get(label, 0)
        if current >= max_count:
            continue
        indices.append(index)
        classwise_count[label] = current + 1
            ...
            eval_specs.append(
                {
                    ...
                    "display_name": get_display_dataset_name(dataset_name),
...
def build_openood_cifar_eval_setup(args, preprocess):
    openood_root = resolve_openood_root(args)
    benchmark_dir = resolve_openood_cifar_benchmark_dir(openood_root, args.in_dataset)
    images_classic_root = openood_root / "data" / "images_classic"
    if not images_classic_root.is_dir():
        raise FileNotFoundError(f"OpenOOD images_classic root does not exist: {images_classic_root}")

    imglists = OPENOOD_CIFAR_IMGLISTS[args.in_dataset]
    id_loader = build_imglist_loader(
        dataset_root=images_classic_root,
        imglist_path=benchmark_dir / imglists["id"],
        preprocess=preprocess,
        batch_size=args.batch_size,
    )

    eval_specs = []
    for group_name, dataset_names in OPENOOD_CIFAR_GROUPS[args.in_dataset].items():
        for dataset_name in dataset_names:
            eval_specs.append(
                {
                    "dataset_name": dataset_name,
                    "display_name": get_display_dataset_name(dataset_name),
                    "group_name": group_name,
                    "stream_name": f"openood_{args.in_dataset.lower()}_{group_name}_{dataset_name}",
                    "ood_loader": build_imglist_loader(
                        dataset_root=images_classic_root,
                        imglist_path=benchmark_dir / imglists[dataset_name],
                        preprocess=preprocess,
                        batch_size=args.batch_size,
                    ),
                }
...
def build_eval_setup(args, preprocess):
    ...
    if args.eval_protocol == "openood_cifar":
        return build_openood_cifar_eval_setup(args, preprocess)
...
def get_display_dataset_name(dataset_name):
    return OPENOOD_DISPLAY_NAMES.get(dataset_name, dataset_name)


def save_metric_rows(args, dataset_rows, summary_rows=None, overall_row=None):
    data = {}
    ...
    if overall_row is None:
        ...
        overall_row = {
            "name": "Overall AVG",
            "fpr": avg_metrics["fpr"],
            "auroc": avg_metrics["auroc"],
            "aupr": avg_metrics["aupr"],
        }
    data[overall_row["name"]] = metric_triplet_to_percent(
        overall_row["fpr"], overall_row["auroc"], overall_row["aupr"]
    )
...
def load_or_build_class_prototypes(args, model, preprocess, log):
    ...
    if train_imglist is not None:
        if args.eval_protocol == "openood_cifar":
            dataset_root = resolve_openood_root(args) / "data" / "images_classic"
        else:
        ...
        if args.in_dataset in OPENOOD_CIFAR_GROUPS:
            dataset_root = resolve_openood_root(args) / "data" / "images_classic" / args.in_dataset.lower() / "train"
            loader = build_subset_imagefolder_loader(
                dataset_root=dataset_root,
                preprocess=preprocess,
                batch_size=args.prototype_batch_size,
                max_count=args.train_shot_per_class,
            )
        else:
    ...
    num_classes = len(get_id_class_names(args))
    device = next(model.parameters()).device
...
def main():
    ...
    positive_labels = [str(label) for label in get_id_class_names(args, val_loader)]
    ...
    for spec in eval_specs:
        ...
        display_name = spec.get("display_name", get_display_dataset_name(out_dataset))
    ...
    summary_rows = []
    if "nearood" in grouped_results and "farood" in grouped_results:
        for group_name, summary_name in (("nearood", "Near-OOD mean"), ("farood", "Far-OOD mean")):
    ...
    save_metric_rows(args, dataset_rows, summary_rows=summary_rows, overall_row=overall_row)
